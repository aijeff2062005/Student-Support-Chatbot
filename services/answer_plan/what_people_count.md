...
Compatibility-safe legacy plan for **WHAT-COUNT Person**.

### CONTRACT ALIGNMENT

Treat this file as legacy alias of `what_count_people` and use canonical compact fields only.

Use only:
- `PRIMARY_FACTS.answer.data.formatted_answer`
- `PRIMARY_FACTS.answer.data.items`
- `PRIMARY_FACTS.answer.data.scope_entity`
- `PRIMARY_FACTS.answer.data.total` (verification only)
...
- Never state or imply numeric people count (even for "bao nhiêu").
- Group semantics:
...
2. Only add/adjust minimal details from `answer.data.items` and count/pagination fields when needed for strict correctness.
3. If the resolved people scope/group is clearly different from what the user asked, mismatch response overrides normal people-count behavior.

### CONSTRAINTS

- Answer by listing matching people; no numeric count in final response.
- Do not rely on legacy keys (`list_results`, `context_found_list`, ...).

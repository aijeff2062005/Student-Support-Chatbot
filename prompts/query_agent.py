...
- **Support & Activity Entities:** `Facility (Cơ sở vật chất)`, `Certificate (Chứng chỉ)`, `Reward (Giải thưởng)`, `Ranking (Xếp hạng)`, `Activity (Hoạt động)`
...
- **`relation`**: Verification or retrieval of the relationship between **2 or more already-identified entities** (explicitly named by the user or resolved from conversation context). Use it when the endpoints are known and the user asks whether/how they are related, such as a named Major having a named AdmissionMethod/AdmissionCombination, a named Course being related to another named Course, or a Person/role being related to a Faculty/University. Do NOT use `relation` for unknown target slots (`ngành nào`, `chương trình nào`) or admission score/quota value lookups; those route to `list` / `constraint-list` below.
...
**A. General Policies vs. Admission Methods**
- **Student Support Attribute Exception:** If the user asks for ONE concrete support-policy attribute or consequence about scholarship, tuition payment, late payment, payment status, course registration cancellation, or student support (e.g., "điều kiện để nhận học bổng", "em chậm đóng học phí thì sao", "chưa đóng học phí có bị hủy môn không"), use `intent="attributes"` instead of the generic policy relation route.
  - Set `primary_topic="policy"` for scholarship/student-support rules, or `primary_topic="fee"` for tuition/payment rules.
  - Set `subtopics=["support"]` and add a specific tag when useful: `"scholarship"`, `"tuition"`, `"payment"`, or `"course_registration"`.
  - Set `primary_entities=[{{"label": "Policy", "text": "<support policy phrase from the user>"}}]`.
  ...
  - Do NOT apply this exception to enumeration questions such as "có những loại học bổng nào" (use `list`), HOW/procedure questions such as "cách đóng học phí" (use HOW), or fee amount/scope questions already handled by the relation route.
- **General Policies:** Queries about "học phí", "học bổng", general "quy định" or "chính sách".
...
- Example: "Phong nao xu ly ho so chuyen nganh?" -> `intent="relation"`, `primary_topic="policy"`, `context_entities=[{{"label":"Policy","text":"ho so chuyen nganh"}}]`, `primary_entities=[{{"label":"Department","text":"phong ban xu ly"}}]`, `count_enumerate_targets=[]`, `subtopics=["support","department_support","major_transfer"]`.

...
- Academic & Training Entities: `Major`, `Specialization`, `EducationSystem`, `AcademicProgram`, `Course`, `ProgramObjective`, `ProgramLearningOutcome`, `LearnerAssessmentMethod`, `TeachingAndLearningMethod`, `AdmissionCombination`, `CourseLearningOutcome`, `Skill`
- Support & Activity Entities: `Facility`, `Certificate`, `Reward`, `Ranking`, `Activity`
</entity_taxonomy>
...
3. Tier 3: WHAT (`question_type="WHAT"`)
   - `intent="attributes"`: Properties of a SINGLE entity node (e.g., job placement rate, description, general institution facts).
   - `intent="definition"`: Meaning of a concept (e.g., "PLO là gì?").
   - `intent="relation"`: Relationship verification or retrieval between 2+ already-identified entities.
...
A. Admission, Policies & Combinations (Strict Routing)
   - Student Support Domain Priority: For WHAT questions that ask a definition, attribute, condition, consequence, eligibility, timing, responsible unit, status, or detail about current-student support topics outside admissions, route to the student-support domain first. Use `intent="definition"` or `intent="attributes"`, put the named support concept/entity in `primary_entities`, and include `subtopics=["support", "<specific_support_tag>"]`.
     Specific support tags include: `academic_policy`, `scholarship`, `tuition`, `payment`, `course_registration`, `student_document`, `graduation`, `major_transfer`, `internship`, `event`, `activity`, `club`, `service`, `department_support`, `discipline`, `reward`.
     Examples: "Học bổng chính sách là gì vậy ạ?" -> `primary_topic="policy"`, `primary_entities=[{{"label":"Policy","text":"Học bổng chính sách"}}]`, `subtopics=["support","scholarship"]`; "Điều kiện tham gia sự kiện X là gì?" -> `primary_topic="student_life"`, `primary_entities=[{{"label":"Activity","text":"sự kiện X"}}]`, `subtopics=["support","event"]`; "Thực tập cần điều kiện gì?" -> `primary_topic="career"`, `primary_entities=[{{"label":"Policy","text":"thực tập"}}]`, `subtopics=["support","internship"]`.
   ...
   - Student Support Attribute Exception: if the user asks for ONE concrete support-policy attribute or consequence about scholarship, tuition payment, late payment, payment status, course registration cancellation, or student support (for example "điều kiện để nhận học bổng", "em chậm đóng học phí thì sao", "chưa đóng học phí có bị hủy môn không"), use `intent="attributes"`, `label="Policy"` in `primary_entities`, `primary_topic="policy"` or `"fee"`, and `subtopics=["support"]` plus a specific tag such as `"scholarship"`, `"tuition"`, `"payment"`, or `"course_registration"`. Put the requested fact/consequence in `keyword_attributes`. Do NOT apply this exception to enumeration questions ("có những loại học bổng nào"), HOW/procedure questions ("cách đóng học phí"), or fee amount/scope questions already handled by relation.
     CRITICAL entity-text rule for support-policy attributes: `primary_entities[*].text` MUST preserve the narrowest policy/action phrase that identifies the user's situation, including modifiers such as late, unpaid, overdue, cancelled, warning, blocked, missing, failed, not eligible, not completed, or status-specific wording. NEVER reduce a support-policy consequence/penalty question to a broad domain noun such as "học phí", "học bổng", "đăng ký học phần", "thi", "hồ sơ", or "dịch vụ".
...
- Example: "Phòng nào xử lý hồ sơ chuyển ngành?" -> `intent="relation"`, `primary_topic="policy"`, `context_entities=[{{"label":"Policy","text":"hồ sơ chuyển ngành"}}]`, `primary_entities=[{{"label":"Department","text":"phòng ban xử lý"}}]`, `count_enumerate_targets=[]`, `subtopics=["support","department_support","major_transfer"]`.
